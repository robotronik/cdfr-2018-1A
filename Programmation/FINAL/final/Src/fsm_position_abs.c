...
extern machineEtat.deplacement.odometrie 
*/
...
void FSM_Pos_Init(FSM_Instance *fsm)
{
  ...
  fsm_pos->n=0;
  fsm_pos->ntime=abs((fsm_pos->pos/fsm_pos->linear_speed)/(machineEtat.deplacement.asserv.pid_sum).Te);
  fsm_pos->initial_sum=(machineEtat.deplacement).asserv.sum_goal; //0 au dep
  fsm_pos->final_sum=fsm_pos->initial_sum+(double) fsm_pos->pos / (double)ENCODER_STEP_DIST;
  fsm->run=FSM_Pos_Generator;
}
void FSM_Pos_Generator(FSM_Instance *fsm)
{
  FSM_Position_Abs *fsm_pos=(FSM_Position_Abs *) fsm;
  (machineEtat.deplacement).asserv.sum_goal=ramp_generator(fsm_pos->final_sum, fsm_pos->initial_sum, &(fsm_pos->n),fsm_pos->ntime);
  if(reached(&(machineEtat.deplacement.asserv.pid_sum),fsm_pos->final_sum- 0.5 * (machineEtat.deplacement.odometrie.encoder_l.steps + machineEtat.deplacement.odometrie.encoder_r.steps))) fsm->run=FSM_Abs_End;
}
...
void FSM_Angle_Init(FSM_Instance *fsm)
{
  ...
  fsm->status=FSM_RUNNING;
  fsm_pos->n=0;
  fsm_pos->ntime=abs((fsm_pos->angle/fsm_pos->angular_speed)/(machineEtat.deplacement.asserv.pid_diff).Te);
  fsm_pos->angle=-fsm_pos->angle-machineEtat.deplacement.odometrie.theta;//no value limitation
  fsm_pos->initial_diff=machineEtat.deplacement.odometrie.encoder_r.steps - machineEtat.deplacement.odometrie.encoder_l.steps;
  fsm_pos->final_diff=fsm_pos->initial_diff+fsm_pos->angle*ENCODER_DIST/ENCODER_STEP_DIST;
...
{
  FSM_Position_Abs *fsm_pos=(FSM_Position_Abs *) fsm;
  machineEtat.deplacement.asserv.diff_goal=ramp_generator(fsm_pos->final_diff, fsm_pos->initial_diff, &(fsm_pos->n),fsm_pos->ntime);
  if(reached(&(machineEtat.deplacement.asserv.pid_diff),fsm_pos->final_diff - (machineEtat.deplacement.odometrie.encoder_r.steps - machineEtat.deplacement.odometrie.encoder_l.steps))) fsm->run=FSM_Abs_End;
}
