...
def drawSweep(S, x):
    ...
    for i in range(x-5):
        xl1 = sweep_length * cos(angle-i*sweep_speed) + screen_width/2
        yl1 = -sweep_length * sin(angle-i*sweep_speed) + screen_height/2 - border_down
...
def drawText(text1, x1, y1, text2, x2, y2, text3, x3, y3, text4, x4, y4):
...
while (continuer == 1):
    ...
    if (i != n1 and theta1[i] == int(degrees(angle))):
        ...
        i+=1
        # time.sleep(0.2)
        text1 = "Angle1 : " + str(int(degrees(angle))) + deg
        text2 = "Distance1 : " + str(dist) + " mm"
    ...
    drawText(text1, 10, 6, text2, 145, 6, text3, 360, 6, text4, 500, 6)
