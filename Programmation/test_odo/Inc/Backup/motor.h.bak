...
#define DRIVE_MOTOR(voltage, side_port, side_pin, fwd_state, tim_channel) {	\
  int16_t value = (int16_t) (voltage * (255.0 / 12.0));		\
  									\
  if(voltage > 0){							\
    HAL_GPIO_WritePin(side_port, side_pin, 0);	\ // a check normalement fwd_state
  }									\
  else{									\
    HAL_GPIO_WritePin(side_port, side_pin, (~fwd_state)&1);		\ // (~fwd_state)&1 faut changer sidepin
    value = -value;							\
  }									\
									\
  if(value > PWM_MAX) value = PWM_MAX;					\
  sConfigOC.Pulse = value;						\
  									\ // mettre le pin a mettre a mettre en pwm logiciel ca se fait da,s tim_channel1
  HAL_TIM_PWM_ConfigChannel(&htim3, &sConfigOC, tim_channel); \ // changer le htim
